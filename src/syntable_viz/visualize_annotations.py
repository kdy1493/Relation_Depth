"""Visualise SynTable COCO annotations (RGB, depth, masks, occlusion order).

Vendored from https://github.com/ngzhili/SynTable (visualize_annotations.py).
Local extensions: --image-ids, --occ-order.
"""
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument('--image-ids', type=str, default=None,
                        help='Comma-separated COCO image_ids (default: all)')
    parser.add_argument('--occ-order', action='store_true',
                        help='Render occlusion order graph (OOAM) and adjacency heatmap')
    ...
    "occluded":[],"occluded_rate":[],"category_id":[],"object_name":[]}})
            ...
            referenceDict[image_id].update({"rgb": img_rec["file_name"]})
            referenceDict[image_id].update({"depth": img_rec["depth_file_name"]})
            referenceDict[image_id].update({"occlusion_order": img_rec["occlusion_order_file_name"]})
    ...
    if args.image_ids:
        query_img_id_list = [int(x.strip()) for x in args.image_ids.split(",") if x.strip()]
    else:
